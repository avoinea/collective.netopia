...
from collective.netopia.mobilpay.payment.request.notify import Notify
...
class BaseRequest:
    ...
    """
        * signatue(Mandatory) - signature received from mobilpay.ro that
    ...
    """
        * returnUrl(Optional) - URL where the user is redirected from mobilpay.ro
          payment interface
        * when the transaction is canceled or confirmed. If it is not supplied the
          application will use
        * return URL configured in control panel
        * use it only if you want to overwrite the configured one otherwise set
          it's value to NULL
        * var string
    """
    _returnUrl = None
    ...
    """
        * confirmUrl(Optional) - URL of the seller that will be requested when
          mobilpay.ro will make
        * a decision about payment(e.g. confirmed, canceled). If it is not supplied
    ...
    """
        ...
        * in this property is stored the envelope key after encrypting data to
    ...
    def _parse_from_xml(self, element):
        ...
        if attr is None or len(str(attr)) == 0:
            raise Exception(
                "parse_from_xml failed -> empty order id ",
        ...
        if len(elems) != 1:
            raise Exception(
                "parse_from_xml failed -> signature is missing ",
