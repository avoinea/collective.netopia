...
class Notify:
    ...
    def create_xml_element(self, document):
        ...
        self._crc = hashlib.md5(
            str(int(random.random() * int(time.time()))).encode("utf-8")
        ).hexdigest()
        ...
        if self.pan_masked is not None:
            xml_notify.appendChild(
                self.create_text_element(xml_notify, "pan_masked", self.pan_masked)
            )
        ...
        if self.customer_id is not None:
            xml_notify.appendChild(
                self.create_text_element(xml_notify, "customer_id", self.customer_id)
            )
        ...
        if self.paidByPhone is not None:
            xml_notify.appendChild(
                self.create_text_element(xml_notify, "paid_by_phone", self.paidByPhone)
            )
        ...
        if self.installments is not None:
            xml_notify.appendChild(
                self.create_text_element(xml_notify, "installments", self.installments)
            )
