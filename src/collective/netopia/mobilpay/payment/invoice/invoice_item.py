...
class InvoiceItem:
    ...
    def load_from_xml(self, item_node):
        ...
        if len(xml_temp) != 1:
            raise Exception(
                "load_from_xml -> Invalid code element",
        ...
        if len(xml_temp) != 1:
            raise Exception(
                "load_from_xml -> Invalid name element",
        ...
        if len(xml_temp) != 1:
            raise Exception(
                "load_from_xml -> Invalid quantity element",
        ...
        if len(xml_temp) != 1:
            raise Exception(
                "load_from_xml -> Invalid price element",
        ...
        if len(xml_temp) != 1:
            raise Exception(
                "load_from_xml -> Invalid vat element",
    ...
    def create_xml_document(self, document):
        ...
        xml_item.appendChild(
            self.create_cdata_element(document, "measurement", self._measurement)
        )
        xml_item.appendChild(
            self.create_text_element(document, "quantity", self._quantity)
        )
