...
from collective.netopia.mobilpay.payment.invoice.invoice_item import InvoiceItem
...
class Invoice:
    ...
    def create_xml_element(self, document):
        ...
        if (
            self._selectedInstallments is not None
            and len(self._selectedInstallments) > 0
        ):
    ...
    def __str__(self):
        return """{0},{1},{2},{3}""".format(
            self._currency, self._selectedInstallments, self._details, self._amount
        )
