...
from collective.netopia.mobilpay.util.encrypt_data import Crypto
...
class Request:
    ...
    def factory_from_encrypted(
        self, env_key, enc_data, private_key_file_path, private_key_password=None
    ):
        private_key = None
        ...
        if private_key is False:
            raise Exception(
                "Error loading private key", self.ERROR_CONFIRM_LOAD_PRIVATE_KEY
            )
        ...
        if src_data is False:
            raise Exception(
                "Failed decoding data", self.ERROR_CONFIRM_FAILED_DECODING_DATA
            )
        ...
        if len(root_order) != 1:
            raise Exception(
                "factory_from_xml -> order element not found",
